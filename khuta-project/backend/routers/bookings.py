...
def format_validated_ticket(ticket):
    return {
        "id": ticket["id"],
        "seat_number": ticket["seat_number"],
        "user_name": ticket["user_name"],
...
def clean_qr_token(qr_value: str):
    token = qr_value.strip()
...
def validate_qr(data: ValidateQRRequest):
    ...
    try:
        token = clean_qr_token(data.qr_value)
        ...
        return {
            ...
            "ticket": format_validated_ticket(ticket)
        }
